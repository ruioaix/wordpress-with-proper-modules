...
add_action( 'admin_menu', 'rundown_admin_menu' );
function rundown_admin_menu() {
    add_theme_page( 'Rundown Options', 'Rundown Options', 'manage_options', 'rundown-settings', 'rundown_options_page' );
}


/* Create Sections and Fields */

add_action( 'admin_init', 'rundown_admin_init' );
function rundown_admin_init() {

	/* Register setting */
    register_setting( 'rundown-group-1', 'rundown_magazine' );
    register_setting( 'rundown-group-1', 'rundown_excerpt' );
    register_setting( 'rundown-group-1', 'style_setting' );
	
	
	/* Create setting section */ 
    add_settings_section( 'rundown-section-one', __('Layout', 'rundown'), 'section_one_callback', 'rundown-section-1' );
    add_settings_section( 'rundown-section-two', __('Presentation', 'rundown'), 'section_two_callback', 'rundown-section-1' );
	
	/* Create fields */
    add_settings_field( 'field-rundown-one', __('Magazine style', 'rundown'), 'rundown_magazine_setting', 'rundown-section-1', 'rundown-section-one' );
    add_settings_field( 'field-rundown-two', __('Limit excerpt', 'rundown'), 'rundown_excerpt_setting', 'rundown-section-1', 'rundown-section-one' );
    add_settings_field( 'field-rundown-three', __('Style', 'rundown'), 'rundown_style_setting', 'rundown-section-1', 'rundown-section-two' );
...
function rundown_options_page() {
    ...
    <div class="wrap">
        ...
        <form action="options.php" method="POST">
            <?php settings_fields( 'rundown-group-1' ); ?>
            <?php do_settings_sections( 'rundown-section-1' ); ?>
            <?php submit_button(); ?>
        </form>
